...
class VerticalGradientDivider extends StatefulWidget {
  ...
  final int repation;
  const VerticalGradientDivider({super.key, this.width, this.height, required this.color1, required this.color2,  this.repation =1});

  @override
  State<VerticalGradientDivider> createState() => _VerticalGradientDividerState();
}
...
class _VerticalGradientDividerState extends State<VerticalGradientDivider> {
  List<Color> getColorsList(){
    ...
    for(int i = 0 ; i < widget.repation ; i++){
      if(i%2==0) colors.add(widget.color1);
      else colors.add(widget.color2);
    }
    return colors;
  }
  @override
